...
export class MessageListComponent implements OnInit {
  ...
  setUpSocketConnection(): void {
    ...
    this.socket.on('send-message', (message: IMessageContent) => {
      ...
      if (messageData.sender.username === this.connectedUser.username) {
        ...
        this.otherBubble(messageData.message, messageData.timestamp, messageData.sender.username);
      }
    });

    this.socket.on('user-disconnected-error', () => {
  ...
  sendMessage() {
    if (this.sentMessage.trim()) {
      ...
      const user: IUser = { username: this.connectedUser.username, socketId: ''};
      const message: IMessageContent = { message: this.sentMessage.trim(), sender: user, timestamp: '' };
